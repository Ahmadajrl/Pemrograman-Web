...
<div class="bg-white rounded shadow-md overflow-hidden p-6 max-w-2xl">
    ...
    <form action="<?= base_url('admin/barang/update/' . $barang->id) ?>" method="post" enctype="multipart/form-data">
        <div class="mb-4">
            ...
            <input type="text" name="id_barang" class="w-full px-3 py-2 border rounded bg-gray-100 focus:outline-none" value="<?= $barang->id_barang ?>" readonly>
            <p class="text-xs text-gray-500 mt-1">ID Barang tidak dapat diubah.</p>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 mb-2">Nama Barang</label>
            <input type="text" name="nama_barang" class="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-600" value="<?= old('nama_barang', $barang->nama_barang) ?>" required>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 mb-2">Merk Barang</label>
            <input type="text" name="merk_barang" class="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-600" value="<?= old('merk_barang', $barang->merk_barang) ?>" required>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 mb-2">Gambar Produk Baru (Opsional)</label>
            <?php if ($barang->gambar && $barang->gambar !== 'default.png'): ?>
                <img src="<?= base_url('uploads/barangs/' . $barang->gambar) ?>" alt="Gambar" class="h-24 w-24 object-cover mb-2 rounded border">
            <?php endif; ?>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="stok" class="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-600" value="<?= old('stok', $barang->stok) ?>" required>
        </div>
